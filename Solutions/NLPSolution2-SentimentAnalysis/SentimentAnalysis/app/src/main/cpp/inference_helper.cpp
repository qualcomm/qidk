...
//        LOGI("Filling %s buffer ", name);
        ...
        if(bitWidth == 8 || bitWidth == 16) {
            ...
            unsigned char stepEquivalentTo0;
            float quantizedStepSize;
            if (!loadByteDataFileBatchedTfN(inVector[j], arrayLengths[j], applicationBuffers.at(name), 0,
            ...
            loadVector.resize( arrayLengths[j] * 4);
            ...
            for (int idx=0; idx < arrayLengths[j]; ++idx) {
                accumulator[idx] = inVector[j][idx];
//                LOGI("%d -> %f", idx, accumulator[idx]);
            }
//                accumulator[idx] = static_cast <double> (inVector[j][idx]);
        }
    }
    return true;
}
...
//                std::vector<uint8_t> output;
            ...
            } else output = applicationOutputBuffers.at(name);
        }
    }
    return true;
