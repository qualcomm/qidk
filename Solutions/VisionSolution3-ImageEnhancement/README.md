...
- For better performance on Snapdragon SM8850 model quantization is recommended. For quantization please keep the dlc, data and list.txt in one directory. 
"data" has some raw input for model quantization.
"list.txt" has names of all files in data directory

```python
snpe-dlc-quantize --input_dlc enlighten_fixed.dlc --input_list list.txt --use_enhanced_quantizer --use_adjusted_weights_quantizer --axis_quant --output_dlc enlight_axisQ_cached.dlc --enable_htp --htp_socs sm8850
```
