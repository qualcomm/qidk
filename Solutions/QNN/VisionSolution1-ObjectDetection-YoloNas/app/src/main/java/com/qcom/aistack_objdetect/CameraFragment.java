...
public class CameraFragment extends Fragment
{
     ...
     */
    private QNNHelper mQnnHelper;
    public long tic = 0,tic2=0;
    private boolean mNetworkLoaded;

    private ProgressDialog dialog_model_error;
...
//            System.out.println("Textureiew widthxheight: "+mTextureView.getWidth()+ mTextureView.getHeight());
    ...
    private void ensureNetCreated() {
        if (mQnnHelper == null) {
            // load the neural network for object detection with QNN
            mQnnHelper = new QNNHelper(getActivity().getApplication());

            // Show UI dialog till the model is not loaded yet.
